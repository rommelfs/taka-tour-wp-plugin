	...
	<?php if ( ! empty( $drawers ) ) : ?>
		<div class="taka-ticket-info-actions" aria-label="<?php echo esc_attr__( 'Ticket information', 'taka-platform' ); ?>">
			<?php foreach ( $drawers as $drawer_key => $drawer ) : ?>
				<?php $drawer_id = 'taka-info-drawer-' . $event_key . '-' . sanitize_html_class( (string) $drawer_key ); ?>
				<button type="button" class="taka-ticket-info-button" data-taka-info-drawer-open="<?php echo esc_attr( $drawer_id ); ?>">
		...
		<?php foreach ( $drawers as $drawer_key => $drawer ) : ?>
			<?php
			$drawer_id = 'taka-info-drawer-' . $event_key . '-' . sanitize_html_class( (string) $drawer_key );
