...
class TAKA_Platform_Admin {
	...
	public static function render_settings() {
		...
		$location_modes = array( 'list' => __( 'List', 'taka-platform' ), 'flags' => __( 'Flags', 'taka-platform' ), 'map' => __( 'Map view', 'taka-platform' ), 'map_with_list' => __( 'Map with list', 'taka-platform' ) );
		?>
			<div class="wrap">
	...
	public static function render_venue_meta_box( $post ) {
		self::nonce();
		foreach ( array( 'street' => 'Street', 'postal_code' => 'Postal code', 'city' => 'City', 'country' => 'Country', 'country_code' => 'Country code', 'flag' => 'Flag', 'map_x' => 'Map X (0–100)', 'map_y' => 'Map Y (0–100)', 'map_label' => 'Map label', 'timezone' => 'Timezone', 'lat' => 'Geo lat', 'lng' => 'Geo lng' ) as $key => $label ) { self::text( $post->ID, $key, __( $label, 'taka-platform' ) ); }
		self::url( $post->ID, 'website', __( 'Website', 'taka-platform' ) );
	...
	public static function save_venue( $post_id ) { self::save( $post_id, array( 'street', 'postal_code', 'city', 'country', 'country_code', 'flag', 'map_x', 'map_y', 'map_label', 'timezone', 'lat', 'lng', 'website', 'image_id', 'image_url', 'parking_image_id', 'parking_image_url', 'gallery_image_ids', 'parking', 'accessibility', 'notes' ) ); }
	...
	private static function save( $post_id, $fields ) {
		...
		foreach ( $fields as $field ) {
			...
			elseif ( in_array( $field, array( 'map_x', 'map_y' ), true ) ) { $value = is_numeric( $value ) ? (string) max( 0, min( 100, (float) $value ) ) : ''; }
